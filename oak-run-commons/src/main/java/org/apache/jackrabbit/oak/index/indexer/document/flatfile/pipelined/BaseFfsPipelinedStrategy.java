 ...
 */
public class BaseFfsPipelinedStrategy implements SortStrategy {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(BaseFfsPipelinedStrategy.class);
    ...
    private class MonitorTask implements Runnable {
        private final ArrayBlockingQueue<String[]> mongoDocQueue;
        ...
        public MonitorTask(ArrayBlockingQueue<String[]> mongoDocQueue,
    ...
    private void printStatistics(ArrayBlockingQueue<String[]> mongoDocQueue,
    ...
    public BaseFfsPipelinedStrategy(MongoDocumentStore documentStore,
    ...
    public File createSortedStoreFile() throws IOException {
        ...
        // transformThreads is also for baseFFS.
        transformThreads = 1;
        int numberOfThreads = 1+ transformThreads + 1 + 1; // transform, sort threads, sorted files merge
        ...
        try {
            ...
            // baseffs -> transform thread.
            ArrayBlockingQueue<String[]> mongoDocQueue = new ArrayBlockingQueue<>(mongoDocBlockQueueSize);
            ...
            for (int i = 0; i < transformThreads; i++) {
                ...
                BaseFfsPipelinedTransformTask transformTask = new BaseFfsPipelinedTransformTask(
                        docStore,
                        documentNodeStore,
                        Collection.NODES,
                        rootRevision,
            ...
            try {
                ...
                while (tasksFinished < numberOfThreads) {
                    ...
                    try {
                        ...
                        if (result instanceof BaseFfsPipelinedIterateTask.Result) {
                            BaseFfsPipelinedIterateTask.Result downloadResult = (BaseFfsPipelinedIterateTask.Result) result;
                        ...
                        } else if (result instanceof BaseFfsPipelinedTransformTask.Result) {
                            BaseFfsPipelinedTransformTask.Result transformResult = (BaseFfsPipelinedTransformTask.Result) result;
                            ...
                            if (transformTasksFinished == transformThreads) {
                                ...
                                //todo as we are using == in stop thread execution make sure to use these sentinel objects from same classes
                            ...
                            sortedFilesQueue.put(PipelinedStrategy.SENTINEL_SORTED_FILES_QUEUE);
