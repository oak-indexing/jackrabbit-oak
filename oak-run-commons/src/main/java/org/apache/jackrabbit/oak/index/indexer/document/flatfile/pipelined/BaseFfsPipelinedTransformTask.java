...
import static org.apache.jackrabbit.oak.index.indexer.document.flatfile.pipelined.BaseFfsPipelinedStrategy.SENTINEL_MONGO_DOCUMENT;
 ...
 */
class BaseFfsPipelinedTransformTask implements Callable<BaseFfsPipelinedTransformTask.Result> {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(BaseFfsPipelinedTransformTask.class);
    ...
    public BaseFfsPipelinedTransformTask(MongoDocumentStore mongoStore,
                                         DocumentNodeStore documentNodeStore,
                                         Collection<NodeDocument> collection,
                                         RevisionVector rootRevision,
                                         Predicate<String> pathPredicate,
                                         NodeStateEntryWriter entryWriter,
                                         ArrayBlockingQueue<String[]> mongoDocQueue,
    ...
    public Result call() throws Exception {
        ...
        try {
            ...
            while (true) {
                String[] dbObjectBatch = mongoDocQueue.take();
                if (dbObjectBatch == SENTINEL_MONGO_DOCUMENT) {
                    ...
                    for (String dbObject : dbObjectBatch) {
                        ...
                        String value = entry[1];
                                ...
                                if (!NodeStateUtils.isHiddenPath(path) && pathPredicate.test(path)) {
                                    ...
                                    totalEntryCount++;
                                    // Serialize entry
                                    entryWriter.writeTo(writer, path, value);
                                    writer.flush();
                                    ...
                                    nseBatch.addEntry(path, entryData);
                                } else {
